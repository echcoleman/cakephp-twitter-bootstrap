...
/**
 * Bootstrap paginator helper for listings
 */
class TwitterBootstrapPaginatorHelper extends PaginatorHelper {
 ...
 * 
 * Based on Cake PaginatorHelper but compatible with Bootstrap
 *
 ...
 */
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($this->{$check}($model)) {
			...
			unset($options['rel']);
			$text = sprintf($this->_tags['anchor'], $this->_parseAttributes(array_merge($options, compact('escape', 'model'))), $title);
			return $this->Html->tag($tag, $text, array_merge($options, compact('class')));
		}
	}
}